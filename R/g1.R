...
form_g1 <- function(t_g2, num_points = NULL) {
  ...
  t_g1 <-
    dplyr::mutate(t_g1, Scaled = range_scale(Observed, num_points)) # range scale data
  return(t_g1)
}
...
plot_g1 <- function(t_g1) {
  ...
  t_g1_plot <-
    ggplot2::ggplot(t_g1_data, ggplot2::aes(time, Value, color = g1)) +
    ggplot2::geom_point() +
    ...
    ggplot2::geom_hline(yintercept = 0.5,
                        ...
                        color = "blue") +
    ggplot2::geom_vline(
      xintercept = stats::approxfun(x = pred_spl,
                                    y = t_g1$time)(0.5),
      linetype = "dashed",
      color = "blue"
    )
